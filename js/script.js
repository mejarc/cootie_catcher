...
function drawCootie() {
var canvas, ctx, width, height, grad,strokeColor,rand, fillColors;
width=140;
height=140;
strokeColor='#ccc';
...
if($('.cootie canvas', '#main').length) {
 $('.cootie canvas', '#main').each(function(){
   rand = Math.ceil(Math.random()*fillColors.length);
  console.log(fillColors[rand]);
   ctx = $(this)[0].getContext('2d'); //weird. 'Canvas' returns an array?! Blog
   ctx.strokeStyle=strokeColor;
        grad = ctx.createLinearGradient(0, 0, width, height);
grad.addColorStop(0, '#fff');
grad.addColorStop(1, fillColors[rand]); //todo: cycle through bg colors through selectors
ctx.fillStyle = grad;
